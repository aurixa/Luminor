 ...
 */
export function updateCameraPosition(camera: THREE.PerspectiveCamera, player: Player, planet: Planet, deltaTime: number): void {
    // Get player position and direction
    const playerPos = player.getPosition();
    const playerDir = player.getDirection();
    
    // Calculate camera offset based on player direction
    const cameraOffset = playerDir.clone()
