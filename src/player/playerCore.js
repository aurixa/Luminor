...
export function setupPlayer(scene, planet, camera, keys) {
    ...
    const { 
        ...
        directionLine, 
        rightLine 
    } = createAlignmentIndicators(scene);
    ...
    const player = {
        ...
        update: (deltaTime) => {
            ...
            if (keys) {
                ...
                const newPosition = headMesh.position.clone().add(
                ...
                console.log(`Player moved: ${displacement.toFixed(2)} units, New position: X:${headMesh.position.x.toFixed(2)}, Y:${headMesh.position.y.toFixed(2)}, Z:${headMesh.position.z.toFixed(2)}`);
