...
export function setupSegments(scene: THREE.Scene, planet: Planet, headMesh: THREE.Mesh<THREE.SphereGeometry, THREE.Material>): SegmentsSystem {
    ...
    function updateSegments(deltaTime: number): void {
        ...
        for (let i = 1; i < segments.length; i++) {
            ...
            if (pathIndex < path.length) {
                ...
                // Set final position
                const finalPosition = surfacePoint.clone().add(
                    surfaceNormal.multiplyScalar(hoverHeight)
                );
    ...
    function addSegment(): void {
        ...
        if (path.length > 0 && segments.length > 0) {
            ...
            console.log(`Adding segment at path index ${pathIndex}, position: X:${position.x.toFixed(2)}, Y:${position.y.toFixed(2)}, Z:${position.z.toFixed(2)}`);
        } else {
            // Fallback - should never happen
            position = headMesh.position.clone();
            console.log(`FALLBACK: Adding segment at head position: X:${position.x.toFixed(2)}, Y:${position.y.toFixed(2)}, Z:${position.z.toFixed(2)}`);
        }
